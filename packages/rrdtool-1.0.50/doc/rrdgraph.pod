...
=item B<-L>|B<--units-length> I<value> (default 9 characters)
...
rigid boundaries mode.  Normally B<rrdgraph> will automatically expand the
...
By default B<rrdgraph> calculates the width of one pixel in the time domain and
tries to get data at that resolution from the RRD. With this switch you can
override this behavior. If you want B<rrdgraph> to get data at 1 hour
...
In the I<format> string there should be a '%lf', '%lg' or '%le' marker in the
...
be necessary to write two backslashes so that one is passed onto B<rrdgraph>.
...
putting a B<\g> at the end of the string. The B<\g> also squashes any space
...
Whenever B<rrdgraph> gets called, it prints a line telling the size of
